...
@section('content')
                ...
                <div class="row">
                    <div class="col-12">
                        ...
                        <div class="invoice p-3 mb-3">
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <h4>
                                        <img style="height: 50px" src="{{ URL::to('front/assets/images/logo.svg') }}">
                                        Digital BDT
                  ...
                  <img src="../../dist/img/credit/paypal2.png" alt="Paypal"> --}}
                                            ...
                                            @foreach ($deposit_methods as $deposit_method)
                                                <tr>
                                                    ...
                                                    <td style="word-wrap: break-word; width:50%">
                                                        <h6 id="{{ $deposit_method->id }}">{{ $deposit_method->address }}
                                                        </h6>
                                                    </td>
                            ...
                            <div class="callout callout-info">
                                ...
                                To activate your account please pay the equal amount of the package and then submit the
                                transaction hash#
                            </div>
                            <!-- this row will not appear when printing -->
                            <div class="row no-print">
                                <div class="col-12">
                                    <button type="button" data-toggle="modal" data-target="#modal-success"
                                        class="btn btn-outline-success float-right"><i class="far fa-credit-card"></i> Make
                                        Payment
                                    </button>
                                    <button type="button" data-toggle="modal" data-target="#Pay-Using-DBDT"
                                        class="btn btn-outline-primary  float-right" style="margin-right: 5px;"><i
                                            class="far fa-credit-card"></i>
                                        Pay Using DBDT
                                    </button>
                                    <span></span>
                                    <button type="button" onclick="window.print()" class="btn btn-default float-left"
        ...
        <div class="modal-dialog">
            <div class="modal-content bg-success">
                ...
                <div class="modal-body">
                    <form method="POST" action="/user/upgrade-payment">
                        @csrf
                        <div class="card-body">
    ...
    <!-- /.modal -->
    <div class="modal fade" id="Pay-Using-DBDT">
        <div class="modal-dialog">
            <div class="modal-content bg-success">
                <div class="modal-header">
                    <h4 class="modal-title">Pay Using DBDT Confirmation </h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                          ...
                          <div class="table-responsive">
                            <table class="table">
                                ...
                                <tr>
                                  <th>Charge (5%)</th>
                                  @php
                                  $charge = ($package->dbdtprice * 5) / 100;
                                  $subtotal =$charge+ $vat + $package->dbdtprice;
                              @endphp
                                  <td> {{ $charge }} DBDT </td>
                              </tr>
                ...
                <div class="modal-footer justify-content-between">
                    <button type="button" class="btn btn-outline-light" data-dismiss="modal">Close</button>
                    @php
                         $user_balance = DB::table('accounts')->where('user_id', Auth::user()->id)->first();
                    @endphp

                    @if($user_balance->withdraw_balance>=$subtotal)
                    <a href="{{ url('/user/upgrade-payment/using-dbdt') }}" class="btn btn-outline-light">Confirm Payment</a>
                    @else
                    <button class="btn btn-outline-light toastrDefaultError1">Confirm Payment</button>
                    @endif
                </div>
            </div>
            <!-- /.modal-content -->
    ...
    <script>
      $('.toastrDefaultError1').click(function() {
          toastr.error('Sorry!!  You do not have sufficient withdraw balance for payment.')
      });
  </script>
@endsection
